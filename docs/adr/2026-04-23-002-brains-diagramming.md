...
**Revision 3 (2026-04-28):** `er` and `sequence` promoted from reserved v0.5 slots to active types per ADR-003 (`2026-04-28-003-er-and-sequence-diagrams.md`). Two new lazy-on-demand reference files ship (`skills/diagram/references/er.md`, `skills/diagram/references/sequence.md`). The `sequence` auto-trigger heuristic is count-based: ≥2 named participants AND ≥3 ordered message exchanges. SHOULD-level soft caps apply (ER ≤10 entities, sequence ≤6 participants) with `%% NOTE: exceeds SHOULD cap` warning-comment fallback. The full active priority chain is now `state > ER > flowchart > C4 > sequence`. The body of this ADR above this revision history is unchanged.
