...
- Detection is **hybrid**: deterministic bash classifies and counts the changed working-tree file set; the main LLM resolves in-scope documents from the prompt and extracts/follows document links, with `test -f` confirming on-disk dependents. This avoids the council-flagged critical failure of trying to do natural-language scope resolution and robust markdown parsing in bash regex alone.
- Eligibility ceiling: ≤ 4 **target** documents AND ≤ 10 unique on-disk dependent files. Threshold/ineligibility handling is mode-sensitive: **warn-and-ask** interactively, **detect-then-fallback** under `--autopilot`. Manual `--document-mode` on an ineligible change is **asymmetric**: warn-and-confirm for oversized-but-document-only scopes; hard-refuse when actual code **files** are present (fenced/example code *inside* a document never triggers refusal).
...
9. When there is no existing diff (greenfield topic), the main LLM MUST resolve the intended in-scope target documents from the prompt and a repository scan, fix that scope before editing, and re-validate the counts after editing.
...
12. The probe MUST extract document dependencies via the main LLM reading each in-scope document (inline links `[text](path)`, reference definitions `[id]: path`, and image embeds `![alt](path)`), MUST exclude external `http(s)` URLs and pure anchors, MUST resolve repo-relative paths and confirm existence with a filesystem check, and MUST count unique existing dependents toward the ≤ 10 limit.
13. A change MUST be considered eligible for document mode only when it contains zero non-document files (per req 10), AND target documents ≤ 4, AND unique on-disk dependents ≤ 10.
...
15. When auto-detection finds document-only work that exceeds the document or dependent-file ceiling, behavior MUST be mode-sensitive: interactively the system MUST **warn and ask** (surface the counts and offer to proceed via the full `/brains:brains` pipeline or cancel); under `--autopilot` the system MUST **detect-then-fallback**, automatically invoking the full `/brains:brains` pipeline.
...
24. For each reviewed document, the system MUST check word count with `wc -w`: a document under 10,000 words MUST be passed in full to the council; a document at or above 10,000 words MUST be passed as a main-LLM-curated excerpt plus summary rather than in full.
...
32. `references/multi-llm-protocol.md` SHOULD document the document-review variant (`review` with full-document context and the 10,000-word curation gate).
...
flowchart TD
    ...
    B -- yes --> C[Eligibility probe\nhybrid: bash classify + LLM scope/links]
    C --> D{Eligible?\n<=4 docs, <=10 deps,\nno code files}
...
- `skills/document/SKILL.md` and `skills/document/references/eligibility-detection.md` (hybrid detection, link extraction, counting, allow-list).
...
**Test plan:**
- Eligibility: doc-only change at 4 docs (eligible) and 5 docs (over); 10 deps (eligible) and 11 deps (over); a change containing one `.py` file (code present → hard-refuse on manual override); a markdown file containing a fenced `python` block (NOT code present → eligible).
...
- Council review: `--parallel` invokes `star-chamber review` on doc paths; a < 10k-word doc passed full; a ≥ 10k-word doc passed as excerpt+summary; `--single` degrades to self-review with a warning.
...
**Risks accepted:**
- Hybrid detection introduces one LLM-assisted step in the gate (slight latency, mild nondeterminism in scope resolution); mitigated by deterministic numeric counting and filesystem existence checks.
- The 10,000-word gate is below modern context limits (council dissent, below); retained per user direction.
...
2. 🔴 **Critical (integrated):** pure-bash cannot resolve in-scope docs from a natural-language prompt and regex markdown-link parsing is brittle → adopted hybrid LLM-assisted detection (reqs 9, 12).
...
9. ⚖️ **Dissent (overridden by user):** two providers judged the 10,000-word council-review gate an unnecessary relic given modern context windows (~13k tokens). Retained per explicit user direction (req 24); recorded here for the record.
