...
- `--autopilot` — orthogonal to mode; skips user gates and auto-chains phase 1 → 2 → 3. Star-chamber review still runs per selected mode and actionable feedback is auto-integrated; genuine architectural judgment calls surface as `brains:needs-human` tasks during phase 3. At the phase 1 ADR gate, `--autopilot` pauses and presents the gate normally — ADRs encode architectural commitments too consequential to skip blindly. Combine with `--accept-adrs` to also auto-accept the ADR gate. State is persisted in the plan header and honored by `/brains:implement --resume`.
