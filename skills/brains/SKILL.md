...
Priority: `state > ER > flowchart`. When `--max-diagrams 1` (default), dispatch only the highest-priority match. When `--max-diagrams N > 1`, dispatch one diagram per firing heuristic in priority order `state > ER > flowchart > C4 > sequence` (C4 and sequence inactive until ≥2 heuristics fire and their v0.4/v0.5 types are relevant), up to N. When `--diagram <type>` was passed, force exactly that type regardless of heuristics.
